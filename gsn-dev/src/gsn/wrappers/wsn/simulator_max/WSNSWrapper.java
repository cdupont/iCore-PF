...
public class WSNSWrapper extends AbstractWrapper implements DataListener {
      ...
      while ( isActive( ) ) {
         if ( tempStepCounter <= step_counter || step_counter == -1 ) {
            ...
            if ( !listeners.isEmpty( ) && dataBuffer.size( ) > 0 ) {
               ...
               /*
               StreamElement streamElement = new StreamElement( new String [ ] { "NODE_ID" , "PARENT_ID" , "TEMPERATURE" } , new Byte [ ] { DataTypes.INTEGER , DataTypes.INTEGER ,
               ...
               StreamElement streamElement = new StreamElement( new String [ ] { "NODE_ID" , "PARENT_ID" , "TEMPERATURE" } , new Byte [ ] { DataTypes.INTEGER , DataTypes.INTEGER ,
                       DataTypes.DOUBLE } , new Serializable [ ] { dataPacket.getIdentifier( ) , dataPacket.getParent( ) , dataPacket.getRealValue( ) } , System.currentTimeMillis( ) );
                 postStreamElement( streamElement );
   ...
   private static  final DataField[] dataField  = new DataField[] {new DataField( "NODE_ID" , DataTypes.INTEGER_NAME , "Node's identification." ) ,
      ...
      new DataField( "TEMPERATURE" , DataTypes.INTEGER_NAME , "incremental int" )};
   ...
   */
    ...
    private static  final DataField[] dataField  = new DataField[] {new DataField( "NODE_ID" , DataTypes.INTEGER_NAME , "Node's identification." ) ,
          new DataField( "PARENT_ID" , DataTypes.INTEGER_NAME , "Parent Node's identification." ) ,
          new DataField( "TEMPERATURE" , "double" , "incremental double" )};
